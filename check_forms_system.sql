...
-- Check if create_form_recipients function exists (this is what your desktop app calls)
SELECT 
    CASE 
        WHEN EXISTS (
            SELECT FROM information_schema.routines 
...
-- Show sample data from form_submissions table if it exists
SELECT 
    CASE 
        WHEN EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'form_submissions') 
        THEN (SELECT COUNT(*) FROM form_submissions)
        ELSE 0
    END as form_submissions_count;

-- If tables exist, show sample records to understand the data structure
SELECT '=== SAMPLE FORMS DATA ===' as info;
SELECT id, title, description, fields, created_at FROM forms LIMIT 3;

SELECT '=== SAMPLE FORM_RECIPIENTS DATA ===' as info;
SELECT form_id, email, token, created_at FROM form_recipients LIMIT 3;

SELECT '=== SAMPLE FORM_SUBMISSIONS DATA ===' as info;
SELECT form_id, email, responses, submitted_at FROM form_submissions LIMIT 3; 