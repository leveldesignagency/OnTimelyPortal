-- Fix the send_guests_chat_message function to return TABLE format (like GuestChatAdminScreen expects)
-- Drop the JSON version
DROP FUNCTION IF EXISTS send_guests_chat_message(UUID, TEXT, TEXT, TEXT, UUID);

-- Create the function that returns TABLE (matching what GuestChatAdminScreen expects)
