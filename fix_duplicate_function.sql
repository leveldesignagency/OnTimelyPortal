-- Drop the incorrect function versions
DROP FUNCTION IF EXISTS get_guests_chat_reactions_auth(uuid, uuid[]);
DROP FUNCTION IF EXISTS get_guests_chat_reactions_auth(uuid, text, uuid[]);

-- Create the correct function with proper parameter order
