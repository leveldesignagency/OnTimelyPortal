-- Simple Avatar Fix
-- ONLY COMPANY USERS (ADMINS) HAVE PROFILE PHOTOS
-- GUESTS ALWAYS SHOW INITIALS
...
DECLARE
  ...
  v_avatar_url TEXT;
BEGIN
  -- Check if it's a COMPANY USER (ADMIN) - they can have profile photos
  SELECT 
    name,
    'admin',
    company_id,
    avatar_url  -- This will be NULL if no profile photo, or the URL if they have one
  INTO v_sender_name, v_sender_type, v_company_id, v_avatar_url
  FROM users 
  WHERE email = p_sender_email;
  
  -- If not found in users, it's a GUEST - they NEVER have profile photos
  IF v_sender_name IS NULL THEN
    SELECT 
      first_name || ' ' || last_name,
      'guest',
      company_id,
      NULL as avatar_url  -- GUESTS NEVER HAVE PROFILE PHOTOS
    INTO v_sender_name, v_sender_type, v_company_id, v_avatar_url
    FROM guests 
...
-- Test the setup
SELECT 'Simple avatar fix applied!' as status; 