-- UNIFIED CHAT REACTIONS FIX - Clean slate approach
-- This will work for ALL three screens consistently
...
-- 2. Create ONE simple policy that works for everyone
CREATE POLICY "everyone_can_access_reactions" ON guests_chat_reactions
    FOR ALL
    TO public
    USING (
        -- Allow if user is in participants table (admins + guests are ALL in this table)
        EXISTS (
            ...
            AND gcp.user_email = COALESCE(
                ...
                guests_chat_reactions.user_email  -- For direct access
            )
        )
    );

-- 3. Create ONE unified function for getting reactions (all screens will use this)
...
-- 4. Create ONE unified function for adding reactions (all screens will use this)
