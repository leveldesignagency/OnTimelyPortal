-- Fix RLS Avatar Access
-- This fixes the RLS policy issue that's blocking avatar_url access

-- Update the send_guests_chat_message function to bypass RLS
...
DECLARE
  ...
  v_avatar_url TEXT;
BEGIN
  -- First check if it's an admin user (company user) - bypass RLS with SECURITY DEFINER
  SELECT 
    name,
    'admin',
    company_id,
    avatar_url  -- Use avatar_url column for profile photos
...
-- Update the get_guests_chat_messages function to bypass RLS
...
-- Test the setup
SELECT 'RLS avatar access fixed!' as status; 